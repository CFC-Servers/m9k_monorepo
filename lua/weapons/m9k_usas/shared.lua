...
SWEP.WElements              = {
    ["fix2"] = { type = "Model", model = "models/hunter/blocks/cube025x05x025.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector( 22.416, 2.073, -5.571 ), angle = Angle( 0, 0, -90 ), size =
        Vector( 0.899, 0.118, 0.1 ), color = Color( 0, 0, 0, 255 ), surpresslightning = false, material = "", skin = 0, bodygroup = {} },
    ["magfix"] = { type = "Model", model = "models/XQM/cylinderx1.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector( 10.482, 1.389, 0.078 ), angle = Angle( -8.098, 0, 0 ), size = Vector(
        0.2, 0.589, 0.589 ), color = Color( 0, 0, 0, 255 ), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
...
function SWEP:Reload()
    if (self:Clip1() < self.Primary.ClipSize) and (self:GetOwner():GetAmmoCount( "buckshot" ) > 0) and not (self:GetNWBool( "Reloading" )) then
        ...
        timer.Simple( .8, function()
            ...
            if not IsValid( self:GetOwner() ) then return end
            if not IsValid( self ) then return end
            if IsValid( self:GetOwner() ) and self ~= nil then self:ReloadFinish() end
        end )
    end
end
