    ...
    if self:Clip1() > 0 or owner:GetAmmoCount( self:GetPrimaryAmmoType() ) > 0 then
        ...
        timer.Simple( .25, function()
            ...
            timer.Simple( boltactiontime - 0.2, function()
                if not IsValid( self ) or not IsValid( owner ) then return end
                self:SetBoltback( false )
                if owner:KeyDown( IN_ATTACK2 ) and ( not self:IsRunning() or self.CanShootWhileRunning ) and not self:GetReloading() then
...
function SWEP:IronSight()
    ...
    -- Set run effect
    if not selfTbl.CanShootWhileRunning and self:StartedRunning() and not self:GetReloading() then
    ...
    -- Set iron sights
    if ( not self:IsRunning() or selfTbl.CanShootWhileRunning ) and owner:KeyPressed( IN_ATTACK2 ) and not self:GetReloading() and not self:GetBoltback() then
...
function SWEP:DrawHUD()
    ...
    if self:GetIronsights() == true and ( ( not self:IsRunning() or selfTable.CanShootWhileRunning ) and not owner:KeyDown( IN_USE ) ) then
